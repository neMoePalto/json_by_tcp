...
void ParsersManager<DataHeader, AbstractParser>::init() {
  ...
  auto jsonParser = std::make_shared<JsonParser>(
    std::enable_shared_from_this<ParsersManager<DataHeader, AbstractParser>>::shared_from_this());
  ...
  QObject::connect(jsonParser.get(), &JsonParser::messageParsingComplete,
                   _widget.lock().get(), &Widget::printParsingResults);
  QObject::connect(structParser.get(), &StructParser<SomeStruct>::messageParsingComplete,
                   _widget.lock().get(), &Widget::printParsingResults);
}
...
template<typename S, typename PFamily>
std::shared_ptr<ParsersManager<S, PFamily>> ParsersManager<S, PFamily>::create(std::weak_ptr<Widget> w, /*std::shared_ptr<S>*/
                                                                               S header) {
  auto sp = std::shared_ptr<ParsersManager<S, PFamily>>(new ParsersManager<S, PFamily>(w, header));
  sp->init();
...
template<typename S, typename PFamily>
const HeaderDescription<S>* ParsersManager<S, PFamily>::headerDescription() const noexcept {
  return _header.get();
}
...
template<typename S, typename PFamily>
PFamily* ParsersManager<S, PFamily>::chooseParserByDataType(unsigned short type) const {
  const auto it = _dataParsers.find(type);
  if (it == _dataParsers.cend()) {
    return nullptr;
  } else {
...
template<>
const std::map<unsigned short, std::shared_ptr<AbstractP>>*
ParsersManager<EmptyHeader, AbstractP>::parsers() const noexcept {
  return &_dataParsers;
}


template<>
const std::map<unsigned short, std::shared_ptr<AbstractParser>>*
ParsersManager<DataHeader, AbstractParser>::parsers() const noexcept {
  return nullptr;
}
...
void ParsersManager<EmptyHeader, AbstractP>::readMsgFromBeginning(std::string &&data, EmptyHeader* /*ptr*/) {
  ...
  while (data.size() >= 6) {
    ...
    len = htonl(len);
//    qDebug() << "Data len = " << len;
    // Если часть рассматриваемого блока данных находится за пределами текущего сообщения:
    if (len + 6 > data.size()) {
